...
import { generateRecipeAI, generatePantrySuggestionsAI } from '../utils/gemini.js';

export const generateRecipe = async (req, res, next) => {
  try {
    const { ingredients = [], usePantryIngredients = false, dietaryRestrictions = [], cuisineType = 'any', servings = 4, cookingTime = 'medium' } = req.body;
    ...
    res.json({ success: true, message: 'Recipe generated successfully', data: { recipe } });
  } catch (error) {
    next(error);
  }
};
...
export const getRecipes = async (req, res, next) => {
  try {
    ...
    const recipes = await Recipe.findByUserId(req.user.id, {
      search, cuisine_type, difficulty, dietary_tag,
