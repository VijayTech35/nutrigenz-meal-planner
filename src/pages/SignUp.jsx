...
import { Mail, Lock, User, ChefHat, Sparkles, Eye, EyeOff } from 'lucide-react';
...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      toast.success('Account created! Welcome to NutriGenZ!');
      navigate('/dashboard');
    } else {
  ...
  return (
    <div className="min-h-screen flex">
      <div className="flex-1 flex items-center justify-center px-4 py-12 bg-white dark:bg-slate-900">
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">Start your AI-powered nutrition journey</p>
          </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-emerald-900 via-teal-800 to-cyan-900 relative overflow-hidden items-center justify-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative text-center px-12">
          ...
          <h1 className="text-5xl font-bold text-white mb-4">Join NutriGenZ</h1>
          <p className="text-emerald-200/80 text-xl max-w-md mx-auto leading-relaxed">Start your journey to smarter nutrition with AI-powered meal planning.</p>
          <div className="flex flex-wrap justify-center gap-3 mt-10">
            {['AI Recipes', 'Meal Planning', 'Nutrition Tracking', 'Smart Pantry'].map((f, i) => (
              <motion.span key={f} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.1 }}
                className="px-4 py-2 bg-white/5 rounded-xl border border-white/10 text-emerald-200 text-sm font-medium">{f}</motion.span>
            ))}
          </div>
        </motion.div>
      </div>
