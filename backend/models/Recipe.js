...
class Recipe {
  static async create(userId, recipeData) {
    ...
    try {
      ...
      if (ingredients.length > 0) {
        const values = ingredients.map((_, idx) => `($1, $${idx * 3 + 2}, $${idx * 3 + 3}, $${idx * 3 + 4})`).join(',');
        const params = [recipe.id];
        ingredients.forEach((ing) => { params.push(ing.name, ing.quantity, ing.unit); });

        await client.query(`INSERT INTO recipe_ingredients (recipe_id, ingredient_name, quantity, unit) VALUES ${values}`, params);
      }
  ...
  static async findById(id, userId) {
    ...
    const nutritionResult = await db.query(`SELECT calories, protein, carbs, fats, fiber FROM recipe_nutrition WHERE recipe_id = $1`, [id]);

    return { ...recipe, ingredients: ingredientsResult.rows, nutrition: nutritionResult.rows[0] || null };
  }
  ...
  static async findByUserId(userId, filters = {}) {
    ...
    if (filters.search) { count++; query += ` AND (r.name ILIKE $${count} OR r.description ILIKE $${count})`; params.push(`%${filters.search}%`); }
    if (filters.cuisine_type) { count++; query += ` AND r.cuisine_type = $${count}`; params.push(filters.cuisine_type); }
    if (filters.difficulty) { count++; query += ` AND r.difficulty = $${count}`; params.push(filters.difficulty); }
    if (filters.dietary_tag) { count++; query += ` AND $${count} = ANY(r.dietary_tags)`; params.push(filters.dietary_tag); }
    if (filters.max_cook_time) { count++; query += ` AND r.cook_time <= $${count}`; params.push(filters.max_cook_time); }
    ...
    count++; query += ` LIMIT $${count}`; params.push(limit);
    count++; query += ` OFFSET $${count}`; params.push(offset);
